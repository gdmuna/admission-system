...
<div class="three" ref="three" @wheel="handleScroll1" @touchmove="handleScroll33"
  :style="{ backgroundImage: 'url(' + makeimg + ')' }">
  <div class="card">
    <h2 class="bai">{{Worker[0].org}}干事工作证</h2>
    <div class="zqi aaa"><span class="zi">协会</span><span class="you">{{ Worker[0].org }}</span></div><br>
    <div class="zqi"><span class="zi">姓名</span><span class="you">{{ Worker[0].name }}</span></div><br>
    <div class="zqi"><span class="zi">部门</span><span class="you">{{ Worker[0].work }}</span></div><br>
    <div class="zqi"><span class="zi">学号</span><span class="you">{{ Worker[0].stuId }}</span></div><br>
    <div class="zqi aaa" style="color:rgb(29, 13, 43); font-weight: 1200;">下滑扫码入群</div><br>
  </div>
</div>
 ...
 <div class="header" ref="one" @wheel="handleScroll1" @touchmove="handleScroll33">
  <div class="inner-header flex">
    <div class="yno">
        <div class="nnn"><div class="typed-out"><p class="kkk">是否准备好加入<br>{{ Worker[0].org }}</p></div>
          <div class="bu">  
...
import img1 from './img/join2.jpg';
import img2 from './img/join3.jpg';
import axios from 'axios';
  ...
  created() {
    ...
    const parsedString = this.parseURL(currentURL);
    this.urlParam = parsedString
    // 在这里可以进一步处理解析出的字符串，例如更新数据或执行其他操作
    
       // 获取成员详细信息
      axios.post('https://join.gdmuna.com/api/memberManage/getMemberInfo', {'urlParam': parsedString},{
        headers: {
      ...
      .then(response => {
        ...
        //   work:'会长'
        // }]
        this.Worker[0].org = response.data.data[0].org
        this.Worker[0].name = response.data.data[0].stuName
        this.Worker[0].work = response.data.data[0].department
        this.Worker[0].stuId = response.data.data[0].stuId
        this.Worker[0].nn = response.data.data[0].uuid
        this.isAccept = response.data.data[0].admissionStatus

        this.openthree();

        if(response.data.data[0].org === '网络协会')
        {
          this.makeimg = img1;
        }
        else{
  ...
  computed: {
    ...
    isHandle(){
      ...
      // 获取成员列表
      // axios.post('https://join.gdmuna.com/api/memberManage/getMemberList', {},{
      //   headers: {
      //     'secret': 'yJSYTmmNaotEM59A',
      //     'Content-Type': 'application/json'
      //   }
      // })
      //   .then(response => {
      ...
      // 确认加入
      axios.post('https://join.gdmuna.com/api/memberManage/confirmJoin', {'uuid': this.Worker[0].nn},{
        headers: {
          'Content-Type': 'application/json'
        }
      })
    ...
    openthree(){
      ...
      if(this.isAccept == 1){
        console.log('jdiadahodiawhdoiawhiow');
        this.isOpen = 1;
        this.$refs.three.style.height = '100vh';
      }
    }
